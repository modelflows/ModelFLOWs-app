...
#    Vvir=s[:n]*Wh[:n,:]
    ...
    for k in range(0,n):
        if np.linalg.norm(s[k:n],2)/NormS>esvd:
            kk=kk+1
    st.write(f'Spatial complexity: {kk}')
...
#        areal[i]=np.linalg.norm(u[:,i])/np.sqrt(V.shape[1])
    ...
    for m in range(0,np.size(areal)):
        if areal[m]/np.max(areal)>edmd:
            kk3=kk3+1
    st.write(f'Spectral complexity: {kk3}')
...
#    Vvir=s*Wh
    ...
    st.write(f'Spatial dimension reduction: {np.size(sdot)}')
...
def reconst_IT(hatMode,Time,U,S,sv,nn,TimePos,GrowthRate,Frequency):
    ...
    Unondim = U
    print(f'hatTReconst size: {np.shape(hatTReconst)}')
