...
def hosvd_fun(SNAP, varepsilon1, Tensor, TimePos, path0, decision, decision1):
    ...
    if not os.path.exists(f'{path0}/HOSVD_solution/tol_{varepsilon1}/time_modes'):
        os.mkdir(f"{path0}/HOSVD_solution/tol_{varepsilon1}/time_modes")
    ...
    st.info('Calculating time modes')
    time_modes = np.einsum('ijkl, ai, bj, ck -> abcl', S, U[0], U[1], U[2])

    st.info(f'All time mode plots will be saved to {path0}/HOSVD_solution/tol_{varepsilon1}/time_modes')
    for ModComp in range(time_modes.shape[0]):
        for ModeNum in range(time_modes.shape[-1]):
            fig, ax = plt.subplots()
            ax.contourf(time_modes[ModComp,:,:,ModeNum])
            ax.set_title(f'Component {ModComp+1} - Time mode {ModeNum+1}', fontsize = 14)
            ax.set_xlabel('X')
            ax.set_ylabel('Y')
            fig.tight_layout()
            plt.savefig(f'{path0}/HOSVD_solution/tol_{varepsilon1}/time_modes/Comp_{ModComp+1}_timemode_{ModeNum+1}.png')
            plt.close(fig)
    ...
    np.save(f'{path0}/HOSVD_solution/tol_{varepsilon1}/time_modes', time_modes)
