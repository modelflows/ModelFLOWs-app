...
def lstm_model_hp(num_epochs, k, p, dim_x, dim_y, channels_n, training_generator, 
    ...
    tuner = kt.Hyperband(create_model_hp, objective = 'val_loss', max_epochs = 10, factor = 3, directory = 'dir', project_name = 'x', overwrite = True)

    stop_early = tf.keras.callbacks.EarlyStopping(monitor = 'val_loss', patience = 3, min_delta = 0.001)
...
def cnn_model_hp(num_epochs, k, p, dim_x, dim_y, channels_n, training_generator, 
    ...
    tuner = kt.Hyperband(create_model_hp, objective = 'val_loss', max_epochs = 3, factor = 3, directory = 'dir', project_name = 'x', overwrite = True)
...
################################################################################ 
while True:
  model_type = input('Select the model architecture (CNN/RNN). Continue with RNN')
  if not model_type or model_type.strip().lower() == 'rnn':
     model_type = 'rnn'
     break
  elif model_type.strip().lower() == 'cnn':
     model_type = 'cnn'
     break
  else:
     print('\tError: Please select a valid architecture\t')
