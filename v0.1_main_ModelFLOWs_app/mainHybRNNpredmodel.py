...
if hyper == 'Yes':
    ...
    while True:
        shared_dim = input('Select number of shared dims. Continue with 80: ')
        if not shared_dim:
            shared_dim = 80
            break
...
if not filen:
    filen = f'{timestr}_hybRNN_Solution'
else:
    filen = f'{filen}'
...
if output8 == 'yes': # Indicate the number of the variable and time step (nv, nt)
    while True:
        output81 = input(f'Select number of first "n" variables. Continue with all {int(data.shape[-1])}: ')
        if not output81:
...
if hyper == 'No':
    def create_model(in_shape, out_dim, p = 3, shared_dim = 1000, act_fun= act_func1, act_func2 = act_func2, lr = lr, lf = lf):
...
print('\nMinimum val_loss at epoch', '{:d}'.format(idx+1), '=', '{:.6f}'.format(min_loss))
