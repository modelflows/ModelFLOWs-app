...
if dims <= 2:
    print(f'Time modes shape: {time_modes.shape}')
    ...
    ax.contourf(time_modes)
    ax.set_title('Time mode')
    ax.set_xlabel('X')
    ax.set_ylabel('Y')
    plt.show()
...
if decision_2.strip().lower() in ['.mat', 'mat']:
    ...
    mdic1 = {"Time_modes": time_modes}

    file_mat0 = str(f'{path0}/{filen}/Reconstruction.mat')
    file_mat1 = str(f'{path0}/{filen}/time_modes.mat')
...
while True:
    ...
    if dims==4:
        ...
        ax.contourf(time_modes[ModComp,:,:,0,ModeNum])
