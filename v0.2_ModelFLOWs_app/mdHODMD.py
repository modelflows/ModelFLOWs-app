        ...
        for d in d_list:
            ...
            if n_iter>1:            
                for zz in range(0,n_iter):
                    ...
                    if zz != 0:
                        # del SIT,UIT,Frequency,Amplitude,GrowthRate,hatTIT,hatMode,svIT,TensorIT,nnin
                        ...
                        ## Perform HODMD to the reduced temporal matrix hatT:
                        print('Performing HODMD. Please wait...\n')
                        [hatMode,Amplitude,Eigval,GrowthRate,Frequency] = DMDd.hodmd_IT(hatTIT,d,Time,varepsilon1,varepsilon2)
                        print('\nHODMD complete!\n')
    ...
    while True:
        while True:
            ...
            if not case:
                case = 0
                if not decision_2 or decision_2.strip().lower() in ['npy', '.npy']:
                    TensorReconst = data_load.load_data('npy',f'{path0}/{filen}/d_{d_val[case]}_tol_{tol_val[case]}/TensorReconst.npy')
                elif decision_2.strip().lower() in ['.mat', 'mat']:
                    TensorReconst = data_load.load_data('mat',f'{path0}/{filen}/d_{d_val[case]}_tol_{tol_val[case]}/TensorReconst.mat')
                break
            elif case.isdigit():
                if int(case) <= total_cases:
                    case = int(case)-1  
        ...
        if dims == 3:
            ...
            ax[1].plot(Time[:], TensorReconst[y, x, :].real, 'k-x', label = 'Reconstructed Data')
        ...
        if dims == 4:
            ...
            ax[0].scatter(x, y, c='black', s=50)
            ax[1].plot(Time[:], TensorReconst[c, y, x, :].real, 'k-x', label = 'Reconstructed Data')
            ...
            ax[0].scatter(x, y, c='black', s=50)
            ax[1].plot(Time[:], TensorReconst[c, y, x, nz, :].real, 'k-x', label = 'Reconstructed Data')
    ...
    while True:
        while True:
            ...
            if not case:
                case = 0
                if not decision_2 or decision_2.strip().lower() in ['npy', '.npy']:
                    DMDmode = data_load.load_data('npy',f'{path0}/{filen}/d_{d_val[case]}_tol_{tol_val[case]}/DMDmode.npy')
                elif decision_2.strip().lower() in ['.mat', 'mat']:
                    DMDmode = data_load.load_data('mat',f'{path0}/{filen}/d_{d_val[case]}_tol_{tol_val[case]}/DMDmode.mat')
                break
            elif case.isdigit():
                if int(case) <= total_cases:
                    case = int(case)-1  
                ...
                print('\tError: Select a valid number format (must be integer)\n')
        while True:
            ModeNum = input(f'Introduce the mode number to plot (default mode 1). Maximum number of modes is {DMDmode.shape[-1]}: ')
            if not ModeNum:
                ModeNum = 0
                break
        ...
        if dims == 5:
            ...
            while True:
                ...
                if plane.strip().lower() == 'xy':
                    Tensor = Tensor[:, :, :, nz, :]
                    TensorReconst = TensorReconst[:, :, :, nz, :]
                    break
                elif plane.strip().lower() == 'xz':
                    Tensor = Tensor[:, :, 0, :, :]
                    TensorReconst = TensorReconst[:, :, 0, :, :]
                    break
        ...
        while True:
            if decision4 == True:
                ...
                if not vidvel:
                    ...
                    video(TensorReconst, vel, Title = f'Reconstructed data - {titles[vel]}')
                    break
                elif vidvel.isdigit():
