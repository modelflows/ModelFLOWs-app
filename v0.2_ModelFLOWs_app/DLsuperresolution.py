...
Loss function: {lossf}
    ...
    if hyper == 'no':
        ...
        def create_model_1(in_U_dim, in_V_dim, out_dim):
            ...
            X_hat = tf.einsum('ijkl,ikpl->ijpl', XUus_reshape, XVus_reshape)

            m = Model(inputs=[in_U,in_V],outputs= X_hat)
        ...
        model = create_model_1(in_U_dim, in_V_dim, out_dim)
    ...
    if hyper == 'yes':
        ...
        tuner.search([XUds_train,XVds_train],
                    Xr_train,
                    batch_size=bs,
                    epochs=50,
...
Loss function: 'mse'
    ...
    history = model.fit([XUds_train,XVds_train],Xr_train,
                        batch_size=bs,
                        epochs=epoch,
                        validation_split=0.15,
                        verbose=1,
    ...
    t0 = time.time()
    Xr_hat = model.predict([XUds_test, XVds_test]) 
